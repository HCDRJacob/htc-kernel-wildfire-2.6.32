...
int headset_notifier_register(struct headset_notifier *notifier)
{
	...
	case HEADSET_REG_HPIN_GPIO:
		SYS_MSG("Register HPIN_GPIO notifier");
...
static void debug_work_func(struct work_struct *work)
{
	...
	while (hi->debug_flag) {
		...
		SYS_MSG("HPIN %d, ADC %d", hpin_gpio, adc);
		msleep(1000);
	}
}
				...
				const char *buf, size_t count)
{
	...
	if (strncmp(buf, "enable", count - 1) == 0) {
		if (hi->debug_flag) {
			SYS_MSG("Debug work is already running");
			return count;
		}
		if (!debug_wq) {
			debug_wq = create_workqueue("debug");
			if (!debug_wq) {
				SYS_MSG("Failed to create debug workqueue");
				return count;
			}
		}
		SYS_MSG("Enable headset debug");
		mutex_lock(&hi->mutex_lock);
		hi->debug_flag = 1;
		mutex_unlock(&hi->mutex_lock);
		...
		if (!hi->debug_flag) {
			SYS_MSG("Debug work has been stopped");
			return count;
		}
		SYS_MSG("Disable headset debug");
		mutex_lock(&hi->mutex_lock);
		hi->debug_flag = 0;
		mutex_unlock(&hi->mutex_lock);
		if (debug_wq) {
